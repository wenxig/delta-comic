...
<template>
  <div class="w-full bg-(--van-background) pb-[40px]" :class>
    <Waterfall :source="commentStream" ref="waterfall" :class="$props.listClass" class="h-full"
      ...
      <BikaCommentRow :comment="item" :minHeight :isHighlight="item.$_user?._id == uploader" :height="false"
    ...
    <BikaCommentSender ref="commentSender" @afterSend="handleReloadCommit()" :aim-id="$props.id" mode="comics" />
  </div>
  <bikaChildrenComments ref="childrenComments" anchors="low" :uploader :_father @show-user="previewUser?.show" />
