...
<template>
  <Comp.Popup v-model:show="show" position="bottom" overlay round>
    <div class="w-full min-h-60 px-2">
      ...
      <div v-for="[plugin, value] in Object.entries(Object.groupBy(Array.from(uni.image.Image.fork.entries()).map(([key, { urls: forks }]) => {
        ...
        <div v-for="v in value!">
          ...
          <NSelect :options="v.forks.map(v => ({ value: v, label: v }))" :value="v.active"
            @update:value="url => { uni.image.Image.precedenceFork.set(v.key, url); $emit('change') }" />
        </div>
      </div>
    </div>
  </Comp.Popup>
